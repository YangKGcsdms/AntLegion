...
# 一致性向量（§4 互操作契约）
npx tsx conformance/generate.ts   # 重新生成 vectors.json（仅在有意的协议变更时执行——哈希值变化即线格式破坏）
python3 conformance/verify.py     # 独立的跨语言检查：用独立 Python 实现逐字节复现所有已提交的哈希值
```
...
- **服务端配置由环境变量驱动**（`config.ts`，redis.conf 对应物）：`PORT`（28090）、`ANTLEGION_DATA_DIR`（`.data-v2`）、`ANTLEGION_FSYNC`（`always|everysec|no`，默认 `everysec`）、`ANTLEGION_BUS_SECRET`、`ANTLEGION_MAX_DEPTH`（因果深度上限，默认 64）。设置**稳定**的 `ANTLEGION_BUS_SECRET`——未设置时总线每次启动都会生成新的 HMAC 密钥，重启前写入的 `sig` 就无法再被验证。
...
- **`conformance/vectors.json` 是 §4 互操作契约**（而非散文）：TS 测试套件（`test/conformance.test.ts`）和独立 Python 校验器（`conformance/verify.py`）都复现其哈希值 + 折叠输出。仅在有意的协议变更时重新生成，并审查 diff——哈希变化即线格式破坏。
