...
两个智能体，**什么都不共享**——不共享进程、不共享机器、不共享厂商、不共享内存——却需要
对「什么是真的」达成一致。不是对「该做什么」一致：那各自决定。是对**世界**一致：发生了什么、
X 现在的值是多少、它是怎么来的、它引发了什么、它可不可信。今天，这类智能体之间的介质
是一个人，把状态从一个窗口粘贴到另一个窗口。

这份协议就是那个介质。它是一条**共享世界状态的日志**：每个智能体把自己观察到的沉积上去，
每个智能体——按自己的节奏、在自己的节点上——把同一条日志折叠成同一个世界。其中没有
任何一条是命令，因为命令有收件人，而日志没有。它的血统是「共识环境」（stigmergy）：蚂蚁
不互相寻址，它们读地面。这里的地面是一条全序、只追加、内容寻址的日志；「读地面」是一个
确定性折叠（§3）。

工作流引擎或编排器做的一切都*不在范围内*：这份协议没有步骤、没有指派、没有调度器。它
只有一个看起来像协调的推论——一条事实的所有权本身也是一块世界状态，而全序让它没有
歧义（§3.1）——但那是共享一个世界的推论，不是目的。
