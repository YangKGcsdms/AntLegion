...
**持久化让 Agent 共享现实。认领让它们分工。因果让工作流涌现。** 系统里其余的一切都是这三者之一，都读自同一条有序日志——持久化是只追加日志（[§11.1](../PROTOCOL.zh-CN.md)），领取是最小 seq 定理（[§8.4](../PROTOCOL.zh-CN.md)，证明见 [§9.1](../PROTOCOL.zh-CN.md)），因果是 `refs.parent` 链（[§8.2](../PROTOCOL.zh-CN.md)）。
