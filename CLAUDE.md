...
node dist/bin.js <cmd>  # alctl CLI (publish/read/claim/resolve/state/info); needs build + a running bus
...
- **`server.ts`** — Hono wire surface: `POST /facts`, `GET /facts` (since/type/author/refs), `/facts/head`, `/facts/:id`, `/info` (INFO), `POST /admin/rewrite` (BGREWRITEAOF).
...
- `EVOLUTION.md` — why the project looks like this (v0 runtime → v1 → v2 monist redesign, and why v1 was removed).
