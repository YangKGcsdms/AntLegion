/**
 * demo-worker — one independent agent process for `antlegion demo` (the killer demo).
 *
 ...
 * src/demo.ts (mirrors src/fold.ts semantics: claim/resolve/
 * release/tombstone with recv-anchored deterministic expiry).
 *
 * Normally forked by src/demo.ts as a compiled child process:
 *   node dist/demo-worker.js --bus http://localhost:PORT \
