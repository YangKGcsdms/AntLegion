 ...
 *
 * Run:  npx tsx examples/demo-killer.ts
 *
 * Env knobs:
...
import { foldTasks, type TaskView } from "./demo-killer-worker.js";
...
const WORKER = join(HERE, "demo-killer-worker.ts");
...
/**
 * Kill the agent's whole process GROUP. tsx is a supervisor: it spawns the
 * worker as a grandchild and proxies its exit code. Killing the supervisor
 * alone orphans the worker (it keeps publishing facts — a zombie agent).
 * detached:true makes the child a group leader, so -pid takes both down.
 */
...
function spawnAgent(base: string, name: string, framework: string, mode: "racer" | "victim"): Agent {
  ...
  const child = spawn(TSX, args, { cwd: ROOT, stdio: ["ignore", "pipe", "pipe"], detached: true });
...
async function main(): Promise<number> {
  ...
  if (!existsSync(TSX)) { console.error("tsx not found — run npm install first"); return 1; }

  bus = await bootBus();
  const base = `http://localhost:${bus.port}`;
  console.log(`  bus          : ${base}  ${c("green", "● up")}`);
  if (FIXED_PORT) {
    console.log(`  dashboard    : open demo/dashboard.html?bus=${base}&delta=${DELTA} in a browser NOW`);
  }
  ...
  if (KEEP_BUS && PASS) {
    ...
    const child = spawn(TSX, [join(ROOT, "src", "index.ts")], {
    ...
    child.unref();
    console.log(`  dashboard → open ${c("bold", `demo/dashboard.html?bus=http://localhost:${bus.port}&delta=${DELTA}`)}`);
