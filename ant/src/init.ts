...
import { DEFAULT_BUS_URL, type AntConfig } from "./config.js";
...
export async function runInit(): Promise<void> {
  ...
  let worker = (await ask(`act mode — llm (DeepSeek via pi-ai) or simulated?`, workerDflt)).toLowerCase();
  if (worker !== "llm" && worker !== "simulated") worker = workerDflt;
  ...
  const cfg: AntConfig = {
    busUrl,
    watchRoots: [{ root: workspace, origin: "dcu" }],
    worker: worker as "llm" | "simulated",
    ...(worker === "llm" ? { model } : {}),
    autoGate,
  };
